...
class WooCommerce_FAQs {
	...
	/**
	 * The ID of the currently previewed FAQ
	 *
	 * @since    1.0.0
	 *
	 * @var      null/int
	 */
	protected $faq_preview = null;
	...
	/**
	 * Filter WP's query_vars to allow for ours
	 *
	 * @since     1.0.1
	 *
	 * @return    array    The filtered query_vars
	 */
	...
	/**
	 * Process the request before it's gone!
	 *
	 * @since     1.0.1
	 *
	 * @return    null
	...
	function process_request($wp){
		...
		if( array_key_exists( 'faq-preview', $wp->query_vars ) ) {

			$this->faq_preview = $wp->query_vars['faq-preview'];

		}

		if( array_key_exists( 'faq-view', $wp->query_vars ) ) {

			$this->faq_preview = $wp->query_vars['faq-view'];
